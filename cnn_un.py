...
parser.add_argument('--mode', type=int, default=4, metavar='N',
...
def fgsm_test_mc(model, adversary, epsilon = 1):
...
def main():
    ...
    if args.mode == 0:
        ...
        ckpt_standard = torch.load('checkpoint/LeNet_stadard5.pth.tar')
        model_standard.load_state_dict(ckpt_standard['state_dict'])
        test(model_standard)

        ckpt_dropout = torch.load('checkpoint/LeNet_dropout5.pth.tar')
        ...
        ckpt_standard = torch.load('checkpoint/LeNet_stadard5.pth.tar')
        model_standard.load_state_dict(ckpt_standard['state_dict'])
        adv = Adversary(model_standard, 0.1)
        fgsm_test(model_standard, adv)

    elif args.mode == 4:
        ckpt_dropout = torch.load('checkpoint/LeNet_dropout5.pth.tar')
        model_dropout.load_state_dict(ckpt_dropout['state_dict'])
        adv = Adversary(model_dropout, 0.1)
        fgsm_test_mc(model_dropout, adv, epsilon=0.5)
