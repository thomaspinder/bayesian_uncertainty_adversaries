...
```{r}
set.seed(123)
adv_conf <- sample(bcnn_results$adversary_confidence, size = 500, replace = FALSE)
original_conf <- sample(bcnn_results$original_confidence, size=500, replace = FALSE)
...
adv_post_samps <- rnorm(10000, mean = post_means[1], sd=post_means[3])
ori_post_samps <- rnorm(10000, mean = post_means[2], sd=post_means[4])
posteriors <- data.frame(adv=adv_post_samps, ori=ori_post_samps)
posteriors %>% 
  melt() %>% 
  ggplot(aes(x=value, fill=variable)) +
  geom_density(alpha=0.7)
